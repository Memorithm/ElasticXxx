...
fn run(input: &[u8]) -> std::process::Output {
    let mut child = Command::new(env!("CARGO_BIN_EXE_elastic-cli")).arg("admit-capacity")
...
fn cli_runs_shared_controller_and_emits_rejected_decisions_with_failure_exit() {
    ...
    assert_eq!(report["request"], req); assert_eq!(report["final_width"], 2);
    assert_eq!(report["committed"], true); assert_eq!(report["verification"], "Pass");
    req["observation"]["age_milliseconds"] = json!(101);
    let output = run(&serde_json::to_vec(&req).unwrap());
    assert_eq!(output.status.code(), Some(2));
    let rejected: Value = serde_json::from_slice(&output.stdout).unwrap();
    assert_eq!(rejected["reason"], "stale-observation"); assert_eq!(rejected["committed"], false);
...
fn cli_rejects_ambiguous_unknown_oversized_and_overflowed_input() {
    ...
    for invalid in [
        ...
        raw.replace("\"memory_bytes_per_trial\":100", "\"memory_bytes_per_trial\":18446744073709551616"),
        " ".repeat(16385),
    ] { let output = run(invalid.as_bytes()); assert_eq!(output.status.code(), Some(2)); assert!(output.stdout.is_empty()); }
}
