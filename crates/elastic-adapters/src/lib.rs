...
//! declaration → plan → action → verify discipline with real (but local)
//! effects — actual allocations for the RAM budget, licensed width for CPU
//! concurrency — without OS-specific discovery, NUMA migration, or
//! accelerator code.
//!
...
//! 3. planners propose; only [`apply`](ram::RamBudget::apply) style methods
//!    act, and every proposal is re-validated against bounds, step limits,
//!    and invariants immediately before the effect;
//! 4. an adapter may refuse any action that would violate a declared
//!    invariant — planners cannot override refusals.
