//! Shared parser and Rust expander for the Elastic embedded language.
//!
//! The `ElasticResource` derive and function-like `elastic!` language
...
use syn::{
    braced, parenthesized, Attribute, Data, DeriveInput, Ident, LitInt, LitStr, Token, Visibility,
};
...
/// Expand the `ElasticResource` derive from ordinary Rust tokens.
pub fn expand_derive_tokens(
...
/// ```
/// Parse and expand one `elastic! { ... }` invocation.
///
/// This is the single syntax/lowering authority used by both the proc-macro
/// crate and the read-only `cargo elastic expand` frontend.
pub fn expand_elastic_tokens(
    input: proc_macro2::TokenStream,
) -> Result<proc_macro2::TokenStream, syn::Error> {
    let input = syn::parse2::<ElasticDslInput>(input)?;
