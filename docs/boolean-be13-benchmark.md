...
target, and the original policy was successfully restored.
...
exact repetition/path matrices, deterministic order, continuous frequency
samples, policy restoration, process-counter semantics, SHA-256 files, and the
historical collector/helper blobs from the recorded source SHA. The evidence CI
uses full Git history so this source binding can be checked.
