...
if [[ "$FREQ_MODE" == lock-max && "$FREQ_STABLE" == true && "$FREQ_RESTORED" == true ]]; then
...
- comparison-qualified frequency control: **$COMPARISON_QUALIFIED**.
...
(cd "$OUT_DIR" && sha256sum raw.csv frequencies.csv frequency_samples.csv process_metrics.csv metadata.txt README.md > SHA256SUMS)
...
if [[ "$REQUIRE_QUALIFIED" == 1 && "$COMPARISON_QUALIFIED" != true ]]; then
  echo "BE13 evidence did not satisfy the requested qualified frequency-control gate" >&2
  exit 8
fi
